                           ...
                           const unsigned int                level = numbers::invalid_unsigned_int)
{
  ...
  /// need a common interface for mg and active cell iterators
...
/**
 * A make_sparsity_pattern() variant for non-rectangular matrices based on
 * different finite elements for ansatz and test functions but sharing the
 * same triangulation. Distributed triangulations are supported.
 *
 ...
 * implementation of make_sparsity_pattern() for square matrices. Extending
 * this function for (0) requires a deeper understanding of AffineConstraints.
 */
...
template<bool is_multigrid, int dim, int spacedim, typename SparsityPatternType>
void
make_flux_sparsity_pattern_impl(const DoFHandler<dim, spacedim> &    dofh_row,
                                const DoFHandler<dim, spacedim> &    dofh_column,
                                SparsityPatternType &                sparsity,
                                ...
                                const unsigned int level = numbers::invalid_unsigned_int)
{
  ...
  /// Convert entries in int_dof_mask to boolean values so we can pass it to
  ...
  const auto & add_entries_foreach_cell = [&](const auto & range_of_cells_row) {
    for(const auto & cell : range_of_cells_row)
      if(is_locally_owned(cell))
      {
        ...
        /// make sparsity pattern for integrals over this cell
        ...
        for(const unsigned int face_n : cell->face_indices())
        {
          ...
          /// face is interface between two cells
          else
          {
            /// TODO treat periodic neighbors...
            ...
            const unsigned int neighbor_face_n = cell->neighbor_face_no(face_n);
            AssertDimension(neighbor_face_n, cell_column->neighbor_face_no(face_n));
...
/**
 * A make_flux_sparsity_pattern() variant for non-rectangular matrices based on
 * different finite elements for ansatz and test functions but sharing the
 * same triangulation. Distributed triangulations are supported.
 *
 * What's not supported?
