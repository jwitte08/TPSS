...
public:
  ...
  FullMatrix<double>
  compute_prolongation_sf_to_velocity() const;

  void
  prolongate_sf_to_velocity(LinearAlgebra::distributed::Vector<double> &       velocity,
  ...
  /// opposite to the new method one assumed that velocity shape functions/node
  /// functionals are already separated into "interior" and "face". however,
  /// this holds e.g. not for nodal Raviart-Thomas elements...
  std::array<LAPACKFullMatrix<double>, 2>
  compute_nondivfree_shape_functions_old() const;
  ...
  if(is_stream_exact)
  {
    ...
    const auto distribute_local_to_global_impl = [&](const auto & cd) {
      zero_constraints.template distribute_local_to_global<TrilinosWrappers::SparseMatrix>(
        cd.matrix, cd.rhs, cd.dof_indices, system_matrix, system_rhs);
    };
...
ModelProblem<dim, fe_degree>::compute_nondivfree_shape_functions() const
{
  ...
  LAPACKFullMatrix<double> node_value_weights_all;
  node_value_weights_all.reinit(n_interior_nodes_by_pressure + n_faces_per_cell, n_dofs_per_cell_v);
  ...
  /// of all "pressure" shape functions except the constant mode.
  {
    ...
    for(auto i = 0U; i < node_value_weights.m(); ++i)
      for(auto j = 0U; j < node_value_weights.n(); ++j)
        node_value_weights_all(i, j) = node_value_weights(i, j);
  }
  ...
  /// "pressure" function each.
  {
    ...
    for(auto i = 0U; i < node_value_weights.m(); ++i)
      for(auto j = 0U; j < node_value_weights.n(); ++j)
        node_value_weights_all(n_interior_nodes_by_pressure + i, j) = node_value_weights(i, j);
  }
  ...
  trafomatrix_all.reinit(node_value_weights_all.m(), node_value_weights_all.n());
  const auto & [V, invSigma, UT]          = compute_inverse_svd(node_value_weights_all);
  const auto & inverse_node_value_weights = merge_lapack_decomposition(V, invSigma, UT);
  ...
  for(auto i = 0U; i < trafomatrix_rt_to_gradp.m(); ++i)
    for(auto j = 0U; j < trafomatrix_rt_to_gradp.n(); ++j)
      trafomatrix_rt_to_gradp(i, j) = trafomatrix_all(i, j);

  /// test functions associated with each facet (generated by the constant pressure)
  trafomatrix_rt_to_constp.reinit(n_faces_per_cell, n_dofs_per_cell_v);
  for(auto i = 0U; i < trafomatrix_rt_to_constp.m(); ++i)
    for(auto j = 0U; j < trafomatrix_rt_to_constp.n(); ++j)
      trafomatrix_rt_to_constp(i, j) = trafomatrix_all(n_interior_nodes_by_pressure + i, j);
  ...
  // for(auto i = 0U; i < prolongation_matrix.n(); ++i)
  // {
  //   Vector<double> phi_i(prolongation_matrix.n());
  //   phi_i[i] = 1.;
  //   Vector<double> v(prolongation_matrix.m());
  //   prolongation_matrix.vmult(v, phi_i);
  //   // for (auto j = 0U; j < node_value_weights_all.m(); ++j)
  //   {
  //     Vector<double> node_values(node_value_weights_all.m());
  //     node_value_weights_all.vmult(node_values, v);
  //     std::cout << "node_values of curl phi_" << i << std::endl;
  //     remove_noise_from_vector(node_values);
  //     node_values.print(std::cout);
  //     std::cout << std::endl;
  //   }
  // }
   ...
   */
  {
    ...
    const auto copier = [&](const CopyData & copy_data) {
      for(const auto & cd : copy_data.cell_data)
      {
        ...
        constant_pressure_dof_indices[cell_index] = global_dof_index;
      }
  ...
  const LinearAlgebra::distributed::Vector<double> & stream) const
{
  ...
  using cell_iterator_type = typename DoFHandler<dim>::active_cell_iterator;

  AssertThrow(&dof_handler.get_triangulation() == &dofh_v.get_triangulation(),
  ...
  {
    cell_iterator_type cell_sf(&dof_handler.get_triangulation(),
                               cell->level(),
                               cell->index(),
                               &dof_handler);
...
ModelProblem<dim, fe_degree>::compute_prolongation_sf_to_velocity() const
{
  ...
  /// DEBUG Display prolongated/embedded unit stream functions in ParaView.
  {
    ...
    for(auto j = 0U; j < n_dofs_per_cell_sf; ++j)
    {
      ...
      const std::string        suffix         = "phi" + Utilities::int_to_string(j, 3);
      const auto               n_subdivisions = 10U;
      std::vector<DataComponentInterpretation::DataComponentInterpretation>
        data_component_interpretation(dim,
                                      DataComponentInterpretation::component_is_part_of_vector);
  ...
  /// DEBUG Display unit stream functions in ParaView.
  {
    ...
    for(auto i = 0U; i < n_dofs_per_cell_sf; ++i)
    {
      ...
      oss << prefix << "_"
          << "Q" << Utilities::int_to_string(fe_sf.degree) << "_" << suffix << ".vtu";
      std::ofstream output_vtu(oss.str());

      DataOut<dim> data_out;

      StreamVelocityPP<dim> stream_velocity_pp;

      data_out.attach_dof_handler(unit_dofh_sf);
