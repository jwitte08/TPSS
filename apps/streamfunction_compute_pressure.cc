...
main(int argc, char * argv[])
{
  try
  {
    ...
    const auto & dofh_p = stokes_problem.dof_handler_pressure;
    const auto & fe_p   = dofh_p.get_fe();
    ...
    {
      AssertDimension(n_dofs_per_cell_v, dofh_v.n_dofs()); // one cell
    ...
    /// functions (the constant pressure mode is neglected!).
    {
      ...
      fe_values_p.reinit(dofh_p.begin_active()); // one reference cell

      FEValues<dim> fe_values_v(stokes_problem.mapping, fe_v, quad, update_flags);
      fe_values_v.reinit(dofh_v.begin_active()); // one reference cell
      ...
      const auto n_face_dofs_v     = (1 << dim) * fe_v.dofs_per_face;
