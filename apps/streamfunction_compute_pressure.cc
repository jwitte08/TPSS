...
main(int argc, char * argv[])
{
  try
  {
...

    {
      ...
      std::sort(interface_ids.begin(), interface_ids.end(), [](const auto & lhs, const auto & rhs) {
            ...
            const auto & compute_v_i = [&](const unsigned int i, const unsigned int q) {
              ...
              for(auto j = 0U; j < n_interior_dofs_v; ++j)
              {
                ...
                for(auto d = 0U; d < dim; ++d)
                  value[d] += trafomatrix_rt_to_gradp(i, j) * phi.shape_value_component(jj, q, d);
              }
              return value;
            };
...

      {
        ...
        const auto & compute_v_face = [&](const auto &       phi,
          ...
          {
            for(auto d = 0U; d < dim; ++d)
              value[d] += trafomatrix_rt_to_constp(face_no, j) * phi.shape_value_component(j, q, d);
          }
          return value;
        };
        ...
        auto cell_worker =
          [&](const CellIterator & cell, ScratchData<dim> & scratch_data, CopyData & copy_data) {
            ...
            for(auto face_no = 0U; face_no < GeometryInfo<dim>::faces_per_cell; ++face_no)
            {
              ...
              const auto         ncell           = cell->neighbor(face_no);
              const unsigned int interface_index = get_interface_index({cell->id(), ncell->id()});
    ...
    //   std::vector<bool> pressure_is_fixed_on_cell;
    ...
    //     std::vector<types::global_dof_index> dof_indices_on_cell(cellP->get_fe().dofs_per_cell);
    //     cellP->get_active_or_mg_dof_indices(dof_indices_on_cell);
    //     std::vector<types::global_dof_index>
    //     dof_indices_on_ncell(ncellP->get_fe().dofs_per_cell);
    //     ncellP->get_active_or_mg_dof_indices(dof_indices_on_ncell);

    //     if(pressure_is_fixed_on_cell.empty())
    //     {
    //       pressure_is_fixed_on_cell.resize(cell->get_triangulation().n_active_cells());
    //       pressure_is_fixed_on_cell[cell->index()] = true;
    //       // copy_data.cell_rhs[0]                    = 1.; // start value
    // 	  discrete_pressure[dof_indices_on_cell.front()] = 1.;
    //     }
    //     else
    //       AssertDimension(pressure_is_fixed_on_cell.size(),
    //                       cell->get_triangulation().n_active_cells());

    //     const bool is_fixed_on_cell  = pressure_is_fixed_on_cell[cell->index()];
    //     const bool is_fixed_on_ncell = pressure_is_fixed_on_cell[ncell->index()];

    //     if(is_fixed_on_cell && is_fixed_on_ncell)
    //       return;
    //     else if(!is_fixed_on_cell && !is_fixed_on_ncell)
    //       return;
    ...
    //     FEInterfaceValues<dim> fe_interface_values_p(stokes_problem.mapping,
    //                                                  dof_handler_pressure.get_fe(),
    //                                                  fe_interface_values.get_quadrature(),
    //                                                  fe_interface_values.get_update_flags());
    //     fe_interface_values_p.reinit(cellP, face_no, sface_no, ncellP, nface_no, nsface_no);
    //     const unsigned int n_interface_dofs_p = fe_interface_values_p.n_current_interface_dofs();

    //     FEValues<dim> & phi = scratch_data.fe_values;
    //     phi.reinit(cell);
    //     FEValues<dim> nphi(phi.get_mapping(),
    //                        phi.get_fe(),
    //                        phi.get_quadrature(),
    //                        phi.get_update_flags());
    //     nphi.reinit(ncell);
    ...
    //     std::vector<Tensor<1, dim>> nload_values;
    //     {
    //       Assert(load_function, ExcMessage("load_function is not set."));
    //       AssertDimension(load_function->n_components, dim);
    //       const auto & q_points = nphi.get_quadrature_points();
    //       std::transform(q_points.cbegin(),
    //                      q_points.cend(),
    //                      std::back_inserter(nload_values),
    //                      [&](const auto & x_q) {
    //                        Tensor<1, dim> value;
    //                        for(auto c = 0U; c < dim; ++c)
    //                          value[c] = load_function->value(x_q, c);
    ...
    //     AssertDimension(load_values.size(), phi.n_quadrature_points);
    //     AssertDimension(nload_values.size(), nphi.n_quadrature_points);
    ...
    //       {
    //         const auto & f      = nload_values[q];
    //         const auto & dx     = phi.JxW(q);
    //         const auto & v_face = compute_v_face(nphi, nface_no, q);
    //         nresidual += v_face * f * dx;
    //       }
    //     }

    //     const auto &        joint_dof_indices_p =
    //     fe_interface_values_p.get_interface_dof_indices(); std::vector<double>
    //     joint_dof_values_p; for(const auto i : joint_dof_indices_p)
    //       joint_dof_values_p.push_back(discrete_pressure(i));
    ...
    //     const auto & compute_jump_pn = [&](const unsigned int q){
    //       double       value = 0.;
    //       const Tensor<1,dim> n     = fe_interface_values_p.normal(q);
    //       for(auto j = 0U; j < n_interface_dofs_p; ++j) // skip constant mode
    //         value += joint_dof_values_p[j] * fe_interface_values_p.jump(j, q);
    //       return value * n;
    //     };

    //     const auto & fe_face_values_v = fe_interface_values.get_fe_face_values(0);

    //     double pn_dot_v = 0.;
    //     for(unsigned int q = 0; q < fe_interface_values_p.n_quadrature_points; ++q)
    //     {
    //       const auto & dx      = phi.JxW(q);
    //       const Tensor<1,dim> & v_face  = compute_v_face(fe_face_values_v, face_no, q);
    //       const Tensor<1, dim> & jump_pn = compute_jump_pn(q);

    //       pn_dot_v += jump_pn * v_face * dx;
    //     }

    // 	const auto alpha_face = -residual -pn_dot_v;

    // 	if (!is_fixed_on_cell && is_fixed_on_ncell)
    // 	  {
    // 	    discrete_pressure[dof_indices_on_cell.front()] = alpha_face;
    // 	    pressure_is_fixed_on_cell[cell->index()] = true;
    // 	  }
    // 	else if (is_fixed_on_cell && !is_fixed_on_ncell)
    // 	  {
    // 	    discrete_pressure[dof_indices_on_ncell.front()] = alpha_face;
    // 	    pressure_is_fixed_on_cell[ncell->index()] = true;
    // 	  }
    // 	else
    // 	  AssertThrow(false, ExcMessage("Check the implementation..."));
    //   };
    ...
    //   const auto copier = [&](const CopyData & copy_data) {
    //     // empty_constraints.template distribute_local_to_global<Vector<double>>(
    //     //   copy_data.cell_rhs, copy_data.local_dof_indices, discrete_pressure);
    //   };
