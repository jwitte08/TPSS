...
protected:
  ...
  SetUp() override
  {
    if(fe_degree_p == 0)
      GTEST_SKIP()
	<< "Skipping... This test is expected to fail, because there exist no piecewise constant functions with inter-element continuity (if needed use FE_DGQ<dim>(0) instead of FE_Q<dim>(0))...";
