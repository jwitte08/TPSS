...
/*
 * Tests for tensor_product_matrix_new.h:
 *
 * - vmult()
 * - as_table()
 *
...
#include "solvers_and_preconditioners/TPSS/tensor_product_matrix_new.h"
...
//   SetUp() override
//   {
//     ofs.open("tensor_product_matrix_new.log", std::ios_base::app);
...
check_basic_apply_inverse(
  const std::vector<typename Tensors::TensorProductMatrix_new<order, Number>::tensor_type> &
    tensors)
{
  Tester tester;

  Tensors::TensorProductMatrix_new<order, Number> tpm_basic(tensors);
...
{
  /// clear output file
  std::ofstream ofs("tensor_product_matrix_new.log", std::ios_base::out);
  ofs.close();
