 ...
 * ... fast diagonal integrator (vertex patch)!
 *
...
struct Test
{
   ...
   * corresponds to one vertex patch), by extracting blocks from the
   * system matrix. The boolean @p store_block_matrices determines if
   * in addition to the inverse block matrices the "forward" block
   * matrices are stored.
   */
  void
  assemble_smoother_vertex_patch()
  {
    ...
    // *** initialize block list defining the vertex patches
    test_details << "Assembling BlockJacobi-smoother and -submatrices ...  \n\n";
    auto vpatch_additional_data = std::make_shared<typename RELAXATION_BLOCK::AdditionalData>();
    SchwarzSmootherData schwarz_smoother_data;
    schwarz_smoother_data.patch_variant    = TPSS::PatchVariant::vertex;
    schwarz_smoother_data.smoother_variant = TPSS::SmootherVariant::additive;
    vpatch_additional_data->relaxation =
      TPSS::lookup_damping_factor(schwarz_smoother_data.patch_variant,
                                  schwarz_smoother_data.smoother_variant,
                                  dim);
    vpatch_additional_data->inversion = PreconditionBlockBase<double>::svd;
    DoFTools::make_vertex_patches(vpatch_additional_data->block_list,
                                  dof_handler,
                                  global_level,
                                  /*interior dofs only?*/ false);
    if(vpatch_additional_data->block_list.n_rows() > 0)
      vpatch_additional_data->block_list.compress();
    block_smoother_data = vpatch_additional_data;
    ...
    auto vpatch_smoother = std::make_shared<RELAXATION_BLOCK>(/*store_block_matrices*/ true);
    vpatch_smoother->initialize(sparse_matrix, *block_smoother_data);
    block_smoother = vpatch_smoother;
      ...
      [](const SubdomainHandler<dim, double> &                             data,
        ...
        for(unsigned int id = subdomain_range.first; id < subdomain_range.second; ++id)
        {
          ...
          std::array<Table<2, VectorizedArray<double>>, dim> && laplace_matrices =
            fd_eval.patch_action(CELL_ACTION{}, FACE_ACTION{}, FACE_ACTION{});
  ...
  operator()()
  {
    ...
    // TODO to compare patch matrices we have to identify the global
    // dof indices !!!

    assemble_smoother_vertex_patch();
    test_details << "Printing the block matrices (vertex patch) ... \n\n";
    for(unsigned id = 0; id < n_blocks; ++id)
      print_block_matrix(id);
    test_details << std::endl;

    assemble_fast_diagonal_submatrices();
    test_details << "Printing the fast diagonal subspace matrices (vertex patch) ... \n\n";
    ...
    mg_schwarz_smoother_precondition.apply(global_level, dst, src);
...
main(int argc, char * argv[])
{
  ...
  parameters.schwarz_smoother_data.patch_variant = TPSS::PatchVariant::vertex;
