...
template<typename Number>
Table<2, Number>
reshuffle_full(const Table<2, Number> in,
               std::size_t            big_m,
               std::size_t            big_n,
               std::size_t            small_m,
               std::size_t            small_n)
{
  AssertDimension(in.size()[0], big_m * small_m);
  AssertDimension(in.size()[1], big_n * small_n);
  Table<2, Number> ret(big_m * big_n, small_m * small_n);
  for(std::size_t i = 0; i < big_m; i++)
  {
    for(std::size_t j = 0; j < big_n; j++)
    {
      for(std::size_t k = 0; k < small_m; k++)
      {
        ...
        {
          ret(i * big_n + j, k * small_n + l) = in(i * small_m + k, j * small_n + l);
        }
      }
    }
  }
...
template<typename Number>
void
compute_ksvd(const Table<2, Number> &                       in,
...
template<typename Number>
void
compute_ksvd(const AlignedVector<Number> &                  in,
             std::vector<std::array<Table<2, Number>, 2>> & out,
             std::size_t                                    lanczos_iterations = -1)
{
  std::size_t out_rank = out.size();
