...
//calculate the new index (the index in the reshuffled matrix) of the i-th diagonal entry, having a diagonal matrix only makes sense if kronecker factors are square
...
template<typename Number>
AlignedVector<Number>
reshuffled_diag_vector_multiplication(const AlignedVector<Number> & diagonal, const AlignedVector<Number> vec, std::size_t big_m, std::size_t small_m)
{
...
template<typename Number>
AlignedVector<Number>
transpose_reshuffled_diag_vector_multiplication(const AlignedVector<Number> & diagonal, const AlignedVector<Number> vec, std::size_t big_m, std::size_t small_m)
{
             ...
             const std::size_t lanczos_iterations = out_rank * out_rank + 10)
{
	if(dim==2)
	{
		    ...
		    i++)
		{
			...
			v.push_back(vector_inverse_scaling(p.back(), beta.back()));
			r.push_back(vector_scaling(u.back(), -beta.back()));
			r.back() = vector_addition(r.back(),reshuffled_diag_vector_multiplication(in, v.back(), big_m, small_m));
			orthogonalize(r);
			alpha.push_back(std::sqrt(inner_product(r.back(), r.back())));
			u.push_back(vector_inverse_scaling(r.back(), alpha.back()));
			p.push_back(vector_scaling(v.back(), -alpha.back()));
			p.back() = vector_addition(p.back(),transpose_reshuffled_diag_vector_multiplication(in, u.back(), big_m, small_m));
			orthogonalize(p);
		}
		std::size_t           base_len = alpha.size() - 1;
